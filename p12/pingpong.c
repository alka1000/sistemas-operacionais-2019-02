...
int barrier_join (barrier_t *b)
{
  ...
  {
    b->counter++;
    task_suspend(running_task,&(b->queue));
    task_switch(dispatcher_task);
  }
  else
  {   
    printf("Liberar\n"); 
    task_t* aux = b->queue;
    int size = queue_size((queue_t*)(b->queue));
    int i = 0;
    while(i < size)
    {
      aux = (b->queue);
...
int mqueue_send (mqueue_t *queue, void *msg)
{
  ...
  if (sizeof(*msg) != queue->item_size)
  {
    printf("Dados diferentes\n");
    return -1;
  }
  sem_down(queue->sem);
  if(queue->counter < queue->max)
  {
    queue->counter++;

    memcpy(queue->mbuffer, msg, sizeof(*msg));
  }
  else
  {
    task_suspend(running_task,&(queue->queue));
    task_switch(dispatcher_task);
  }
  sem_up(queue->sem);
  return 0; 
...
int mqueue_recv (mqueue_t *queue, void *msg) 
{
  ...
  {
    queue->counter--;
    memcpy(msg,queue->mbuffer,queue->item_size);
  }
  else
  {
    task_suspend(running_task,&(queue->queue));
...
int mqueue_destroy (mqueue_t *queue)
{
  ...
  sem_down(queue->sem);
  task_t* aux = queue->queue;
  int size = queue_size((queue_t*)(queue->queue));
  int i = 0;
  while(i < size)
  {
    aux = (queue->queue);
    queue_remove((queue_t**)&(queue->queue),(queue_t*)aux);
    queue_append((queue_t**)&ready_queue,(queue_t*)aux);
    i++;
  }
...
{
  printf("%d, %ld\n",queue->counter, sizeof(queue->mbuffer)/queue->item_size);
